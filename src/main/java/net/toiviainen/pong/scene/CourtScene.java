...
import static net.toiviainen.pong.PongApplication.*;
...
public class CourtScene extends AbstractScene {
	...
	//Meida sound paddle
	//===================
//	private final AudioClip paddleHitSound;
	...
	public CourtScene(PongApplication application) throws NullPointerException {
		...
		leftPaddle.setFill(Color.ORANGERED);
		...
		rightPaddle.setFill(Color.ORANGERED);
		...
		String p1Name = read("p1Name.txt");
		String p2Name = read("p2Name.txt");

		//adding player names

		player1 = new Text(p2Name);
		player1.setTextOrigin(VPos.CENTER);
		player1.setFont(SMALL_FONT);
		player1.setLayoutX((RESOLUTION_WIDTH - leftPaddle.prefWidth(-1)) / 35);
		player1.setLayoutY(player1.getLayoutY() + 60);
		player1.setFill(Color.LIGHTGREEN);

		player2 = new Text(p1Name);
		player2.setTextOrigin(VPos.CENTER);
		player2.setFont(SMALL_FONT);
		player2.setLayoutX((RESOLUTION_WIDTH - leftPaddle.prefWidth(-1)) / 1.135);
		player2.setLayoutY(player2.getLayoutY() + 60);
		player2.setFill(Color.LIGHTGREEN);

		//adding sound:
//		paddleHitSound = new AudioClip(getClass().getResource("C:\\Users\\APPLE COMPUTER\\Downloads\\javafx-pong-master\\src\\New recording 47.mp3").toExternalForm());
	...
	@Override
	public void tick() {
		// perform only the decrement of the countdown timer if it's been set.
		if (countDown > 0) {
			countDown--;
			return;
		}
